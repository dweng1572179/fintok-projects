...
Browser (Next.js App Router + React Server Components)  — ?_rsc= prefetch, _next/…?dpl=dpl_… (Vercel deploy IDs)
   ...
   ├─ api.mapbox.com / events.mapbox.com  (tiles, fonts, geocoding; token acct "lukebroumand")
...
- **Maps:** Mapbox (public token, account `lukebroumand`); **Street View** via Google; **photos/brochures** on S3. **Auth:** session-based (`/api/auth/me`).
